...
//   - Beat 5 (research/consulting): Google Custom Search (free tier: 100 queries/day)
...
// ---------------------------------------------------------------------------
// BEATS
// Each beat has either `feeds` (RSS) or `googleQueries` (Custom Search) or both.
// ---------------------------------------------------------------------------
...
const BEATS = [
  ...
  },
  {
    ...
    feeds: [
      ...
      "https://techcommunity.microsoft.com/t5/azure-synapse-analytics-blog/bg-p/AzureSynapseBlog/rss",
      "https://feeds.reuters.com/reuters/technologyNews",
    ],
  },
  {
    ...
    feeds: [
      ...
      "https://www.anaplan.com/blog/feed/",
      "https://feeds.reuters.com/reuters/businessNews",
      "https://www.ft.com/rss/home/technology",
    ],
  },
  {
    ...
    // This beat uses Google Custom Search to hit the consulting sites directly
    // since McKinsey, BCG, Gartner etc. don't publish open RSS feeds.
    // Uses 2 queries/day = ~60/month, well within the free 100/day limit.
    googleQueries: [
      "McKinsey BCG Bain Deloitte AI data analytics report 2025",
      "Gartner AI enterprise analytics consulting industry trends",
    ],
    // Supplemental RSS for this beat (still free)
    feeds: [
      "https://hbr.org/rss/topic/analytics",
      "https://hbr.org/rss/topic/ai",
      "https://mitsloan.mit.edu/ideas-made-to-matter/rss.xml",
      "https://www.technologyreview.com/feed/",
    ],
...
async function fetchBeat(beat, apiKey, googleApiKey, googleCseId) {
  ...
  // Collect from Google Custom Search (only beats that have googleQueries)
  ...
  console.log(`    ${rssItems.length} RSS + ${googleItems.length} Google = ${allItems.length} total items`);

  if (allItems.length === 0) throw new Error("no items retrieved");

  // Ask Claude to curate — no tools block, no per-search fee
